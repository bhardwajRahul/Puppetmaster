"""Auto-routing must persist the registry epoch fallback will reload (#203)."""
...
from puppetmaster.static_catalog import curated_to_specs, reconcile_agentic_catalog
...
class GeneratedSwarmRegistryEpochTests(TestCase):
    ...
    def test_catalog_refresh_persists_the_epoch_fallback_will_reload(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(digest, registry_digest(_registry))

    def test_reconcile_reports_refresh_when_rows_change_without_adds(self) -> None:
        curated = curated_to_specs(
            "agentic", "api", [], allowed_providers={"opencode-go"}
        )
        self.assertGreater(len(curated), 0)
        stale = replace(curated[0], tags=list(reversed(curated[0].tags)))
        existing = [stale, *curated[1:]]
        with patch(
            "puppetmaster.providers.available_providers",
            return_value={"opencode-go"},
        ):
            merged, report = reconcile_agentic_catalog(existing)
        self.assertEqual(report.get("action"), "refreshed")
        self.assertFalse(report.get("added"))
        self.assertNotEqual(registry_digest(merged), registry_digest(existing))
