...
class FxRegistrationTests(unittest.TestCase):
    ...
    def test_fx_is_implement_capable(self) -> None:
        ...
        self.assertEqual(IMPLEMENT_ADAPTER_PRIORITY[-1], "fx")
    ...
    def test_fx_is_a_swarm_analysis_adapter(self) -> None:
        from puppetmaster.swarm_launch import SWARM_ANALYSIS_ADAPTERS

        self.assertIn("fx", SWARM_ANALYSIS_ADAPTERS)


class FxAvailabilityTests(unittest.TestCase):
    def test_unavailable_when_cli_missing(self) -> None:
        from puppetmaster.workers import adapter_is_available

        with mock.patch("puppetmaster.diagnostics._fx_cli_installed", return_value=False):
            self.assertFalse(adapter_is_available("fx"))

    def test_available_when_cli_resolves(self) -> None:
        from puppetmaster.workers import adapter_is_available

        with mock.patch("puppetmaster.diagnostics._fx_cli_installed", return_value=True):
            self.assertTrue(adapter_is_available("fx"))

    def test_doctor_reports_optional_when_missing(self) -> None:
        from puppetmaster.diagnostics import _fx_check

        with mock.patch("puppetmaster.diagnostics._fx_cli_installed", return_value=False):
            check = _fx_check()
        self.assertEqual(check.name, "fx")
