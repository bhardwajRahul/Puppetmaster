"""Worker adapter for ``fx ask --json``.

Prompt on stdin. One JSON object on stdout with ``usage.input_tokens`` /
``output_tokens``. No ``--model`` flag: ``payload.model`` becomes ``FX_MODEL``,
and the payload records the model fx reports. MCP is off
(``FX_DISABLE_MCP=1``) unless ``payload.allow_mcp`` is true.
"""
...
def resolve_mcp_disabled(task: Task) -> bool:
    """True unless ``payload.allow_mcp`` is the boolean ``True``.

    Non-booleans fall back to disabled. The switch is an env var so older fx
    ignores it instead of dying on an unknown ``--no-mcp`` flag.
    """
...
class FxAdapter(CliWorkerAdapter):
    """Non-interactive ``fx ask --json`` worker."""
