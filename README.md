...
Puppetmaster is a daily-driver beta at **v1.26.1**, suitable for supervised local engineering. This patch retries parent-side Windows `source_stamp` Access Denied on worker attach/launch. v1.26.0 closed five control-plane kernel gaps: enqueue refuses an already-exhausted job budget, claim/peek can filter by adapter and label capabilities, `update_task_status` enforces a legal transition map, `payload.prerun.skip` marks SKIPPED before adapter spawn, and receipts split working time from parked HOLD/wait wall time.
